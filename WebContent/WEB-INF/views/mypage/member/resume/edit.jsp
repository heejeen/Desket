...
<body
	...
	<div class="app-content content">
		...
		<div class="content-wrapper">
			<div class="content-header row">
				<div class="content-header-left col-md-9 col-12 mb-2">
					<div class="row breadcrumbs-top">
						<div class="col-12">
							<h2 class="content-header-title float-left mb-0">이력서 수정</h2>
								...
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item"><a
										href="/desket/mypage/member/resume/resumelist.do?menuid=resumelist">이력서
											관리</a></li>
									<li class="breadcrumb-item active">이력서 수정</li>
								</ol>
							</div>
						</div>
					</div>
			...
			<form method="POST" action="/desket/mypage/member/resume/editok.do">
						...
						<div class="card-content">
							<div class="card-body" id="letter">
								<fieldset class="form-group">
									...
									<!-- BEGIN : 자소서 내용 입력 -->
									<div id="letterForm">
										<c:forEach items="${letter}" var="letter" varStatus="status">
										<script>
										cnt++;
										</script>
										<textarea class='form-control letterTitle' name='letterTitle" + ${status.count} + "' rows='3' placeholder='자소서 제목'>${letter.title}</textarea>
										<textarea class='form-control letterContent' name='letterContent" + ${status.count} + "' rows='20' placeholder='자소서 내용'>${letter.content}</textarea>
										<script>
										$(".letterTitle, .letterContent").css("resize", "none");
										$(".letterContent").css("margin-bottom", "40px");
										$("#cnt").attr("value", cnt);
										...
										<div class="row">
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumeTitle" class="form-control"
															name="resumeTitle" placeholder="이력서 제목 입력" value="${resume.title}">
													</div>
												</div>
											</div>
											...
											<div class="col-12">
												<div class="form-group row">
													...
													<div class="col-md-8">
														<c:if test="${resume.pic == 'nopic.png'}">
														<input type="text" id="resumePic" class="form-control"
															name="resumePic" placeholder="사진">
														</c:if>
														<c:if test="${resume.pic != 'nopic.png'}">
														<input type="text" id="resumePic" class="form-control"
															name="resumePic" placeholder="사진" value="${resume.pic}">                 
														</c:if>
													</div>
												</div>
											</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumeTask" class="form-control"
															name="resumeTask" placeholder="ex) 웹개발자" value="${resume.task}">
													</div>
												</div>
											</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumeNationality"
															...
															placeholder="ex) 대한민국" value="${resume.nationality}">
													</div>
												</div>
											</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumeMajor" class="form-control"
															name="resumeMajor" placeholder="ex) 컴퓨터공학" value="${resume.major}">
													</div> 
												</div>
											</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumeGrades" class="form-control"
															name="resumeGrades" placeholder="3.3 학점 점수로 변환" value="${resume.grades}">
													</div>
												</div>
											</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumeCareer" class="form-control"
															name="resumeCareer" placeholder="년단위 입력 ex) 1" value="${resume.career}">
													</div>
												</div>
											</div>
											...
											<div class="col-12">
												<div class="form-group row">
													<div class="col-md-4">
														<span class="fspn">희망연봉</span><small>(단위: 만원)</small><span class="point"></span>
													</div>
													<div class="col-md-8">
														<input type="text" id="resumeSalary" class="form-control"
															name="resumeSalary" placeholder="ex) 3200 (단위: 만원)" value="${resume.salary}">
													</div>
												</div>
											</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-8">
														<input type="text" id="resumePortfolio"
															...
															placeholder="포트폴리오 주소 입력" value="${resume.portfolio}">
													</div>
												</div>
											</div>
...
<script>
	...
	lanTemp = "<div class='col-12' style='height:45px;'><div class='form-group row'><div class='col-md-4'></div><div class='col-md-8'><!-- BEGIN : 언어 선택 --><select name='language" + lanCnt + "' id='language" + ${status.count}+ "'class='form-control' style='width: 100%; display: block;'><option value=''>언어선택</option><c:forEach items='${languageList}' var='list'><option value='${list.seq}'>${list.language}</option></c:forEach></select><!-- END : 언어 선택 --></div></div></div></div>";              
	
	$("#languageForm").append(lanTemp);
	$("#lanCnt").attr("value", lanCnt);
	$("#language${status.count}").val("${language}");
	//언어 추가
	
</script>
</c:forEach>
<c:forEach items="${certificate}" var="certificate" varStatus="status">
<script>
	
	//자격증 추가
	cerCnt++;

	cerTemp = "<div class='col-12' style='height:45px;'><div class='form-group row'><div class='col-md-4'></div><div class='col-md-8'><!-- BEGIN : 자격증 선택 --><select name='certificate" + cerCnt + "' id='certificate" + ${status.count}+ "' class='form-control' style='width: 100%; display: block;'><option value=''>자격증선택</option><c:forEach items='${certificateList}' var='list'><option value='${list.seq}'>${list.certificateName}</option></c:forEach></select><!-- END : 자격증 선택 --></div></div></div></div>";              

	$("#certificateForm").append(cerTemp);
	$("#cerCnt").attr("value", cerCnt);
	$("#certificate${status.count}").val("${certificate}");
	//자격증 추가
